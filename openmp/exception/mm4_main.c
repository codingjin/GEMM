...
void ab_par00(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);
void ab_par01(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);
void ab_par10(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);
void ab_par11(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);

void aTb_par00(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);
void aTb_par01(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);
void aTb_par10(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);
void aTb_par11(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);

void abT_par00(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);
void abT_par01(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);
void abT_par10(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);
void abT_par11(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);

void aTbT_par00(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);
void aTbT_par01(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);
void aTbT_par10(const float *__restrict__ A, const float *__restrict__ B, float *__restrict__ C, int Ni, int Nj, int Nk);
...
//	int version = 3;
  for(int version=0; version<4;version++)
  {
   ...
   if (telapsed > maxt_seq) maxt_seq=telapsed;
    ...
    printf("Ni=%d Nj=%d Nk=%d maxt_seq=%.2f mint_seq=%.2f\n", Ni, Nj, Nk, maxt_seq, mint_seq);
    ...
    for (nt=0;nt<num_cases;nt ++)
    {
     ...
     for (trial=0;trial<NTrials;trial++)
     {
      ...
      case 0:
		if (Ni%2 && Nk%2) {// ik 11
			tstart = omp_get_wtime();
			ab_par11(A,B,C,Ni,Nj,Nk);
			telapsed = omp_get_wtime()-tstart;
		}else if (Ni%2==0 && Nk%2) { // ik 01
			tstart = omp_get_wtime();
			ab_par01(A,B,C,Ni,Nj,Nk);
			telapsed = omp_get_wtime()-tstart;
		}else if (Ni%2 && Nk%2==0) { // ik 10
			tstart = omp_get_wtime();
			ab_par10(A,B,C,Ni,Nj,Nk);
			telapsed = omp_get_wtime()-tstart;
		}else { //ik 00
			tstart = omp_get_wtime();
			ab_par00(A,B,C,Ni,Nj,Nk);
			telapsed = omp_get_wtime()-tstart;
		}
		break;
      case 1: 
		if (Ni%2 && Nk%2) {// ik 11
			tstart = omp_get_wtime();
			aTb_par11(A,B,C,Ni,Nj,Nk);
			telapsed = omp_get_wtime()-tstart;
		}else if (Ni%2==0 && Nk%2) { // ik 01
			tstart = omp_get_wtime();
			aTb_par01(A,B,C,Ni,Nj,Nk);
			telapsed = omp_get_wtime()-tstart;
		}else if (Ni%2 && Nk%2==0) { // ik 10
			tstart = omp_get_wtime();
			aTb_par10(A,B,C,Ni,Nj,Nk);
			telapsed = omp_get_wtime()-tstart;
		}else { //ik 00
			tstart = omp_get_wtime();
			aTb_par00(A,B,C,Ni,Nj,Nk);
			telapsed = omp_get_wtime()-tstart;
		}
		break;
      case 2: 
		if (Ni%2 && Nk%2) {// ij 11
			tstart = omp_get_wtime();
			abT_par11(A,B,C,Ni,Nj,Nk);
			telapsed = omp_get_wtime()-tstart;
		}else if (Ni%2==0 && Nk%2) { // ij 01
			tstart = omp_get_wtime();
			abT_par01(A,B,C,Ni,Nj,Nk);
			telapsed = omp_get_wtime()-tstart;
		}else if (Ni%2 && Nk%2==0) { // ij 10
			tstart = omp_get_wtime();
			abT_par10(A,B,C,Ni,Nj,Nk);
			telapsed = omp_get_wtime()-tstart;
		}else { //ij 00
			tstart = omp_get_wtime();
			abT_par00(A,B,C,Ni,Nj,Nk);
			telapsed = omp_get_wtime()-tstart;
		}
		break;
      case 3: 
		...
		if (minn > (Nk<<3)) {
			tstart = omp_get_wtime();
			aTbT_par00(A,B,C,Ni,Nj,Nk);
			telapsed = omp_get_wtime()-tstart;
		}else if (minn >= (Nk>>6)) {
			tstart = omp_get_wtime();
			aTbT_par01(A,B,C,Ni,Nj,Nk);
			telapsed = omp_get_wtime()-tstart;
		}else {
			tstart = omp_get_wtime();
			aTbT_par10(A,B,C,Ni,Nj,Nk);
			telapsed = omp_get_wtime()-tstart;
		}
		break;
      }
